...
- `npm install` can complete on a clean clone; if PowerShell script policy blocks it, the README includes fallback commands.
- `npm start` keeps running; if `3210` or `19168` is already in use, the console clearly shows the occupied PID.
- The UI can reach the main screen on first launch and will not get stuck on the splash screen.
- When the AI engine is not ready, the status bar and work log clearly show Ollama / model state.
- When the Browser runtime is not installed, the Browser tab will not falsely appear available, and it can be installed through the `install-playwright-chromium` SOP.
- When asking for current information such as “today’s weather,” the AI will search, extract source content, and organize the answer instead of only listing links.
- When asking for game guides, the AI will first present the key points, then attach the source article and video links.
- When asking for new game recommendations / new releases, the AI will use the live-query fallback and should not stop at “I’ll check that for you.”
- If the model does not emit an ACTION for web research, the backend will still search, extract the first 2 sources, and hand the result back to the Agent Loop for summarization.
- The chat thinking bubble must show the Agent state: planning, searching live sources, extracting source content, organizing the answer, done.
- The work list, SOP list, and Skills list can be loaded, and failed task executions leave readable logs.
- The title bar “Help → Diagnostics” can show version, ports, Ollama, Browser runtime, AppData path, and the tail of the debug log.

## Clean machine acceptance flow

Recommended for a brand-new Windows user or a clean VM:
...
After opening `http://localhost:3210`, verify:

1. Open “Help → Diagnostics” and confirm HTTP `3210`, Remote TCP `19168`, AppData path, and the debug log are readable.
2. If the AI is not ready, install Ollama / download a model according to the UI task, or switch to another provider in the AI engine settings.
3. If the Browser is unavailable, run the `install-playwright-chromium` SOP.
4. Ask “today’s Taipei weather” and confirm the live query returns a concrete weather summary.
5. Ask “help me find GTA5 money-making tips” and confirm the answer includes guide points and source links.
6. Add one low-risk SOP task and confirm the work log, task status, and failure messages behave normally.
...
## 2026.7.28 fix highlights

- Chalkboard image attachments now use cropped PNGs that preserve handwriting and fine chalk strokes; the longest edge is limited to 1600px.
- Vision models are explicitly instructed to inspect text, charts, arrows, and inserted images; if something is unclear, they must not guess.
- Vision Model auto-detection was expanded, and the app now clearly tells the user to choose a Vision Model or add text when the Provider / model cannot read images.

## 2026.7.16 fix highlights

- Agent Loop: when tool results exist, they must be organized into a readable answer instead of only saying “the requested action has been executed.”
- The long interim-plan bubble was removed; progress is now shown by the thinking bubble’s agent status.
- The “Help” menu provides Diagnostics and About (opens GitHub).
- New games / new releases and live-query requests: if the model returns an empty ACTION, the backend still searches, extracts sources, and falls back to a summarized answer.
- Local task ACTIONs (add task, create SOP, etc.) return a readable summary; the web loop and local loop are split.
- Chalkboard “this month’s new releases” auto-summary: parses multiple `date《game》` entries (up to 7 items + trend summary) instead of only the first half of the first item.
- Chalkboard list/news uses a denser layout, and when content exceeds the screen it continues on the next page automatically.

## 2026.7.15 fix highlights

- Creative and development requests, such as “write a little platform game,” are no longer misclassified as Steam installation tasks by the fallback rule.
- Pressing “Clear” on the Chalkboard permanently clears the content and undo/redo history; Undo will not bring back old data.
- AI Chalkboard summaries are limited to at most 4 de-duplicated points, and forced numeric prefixes are removed to avoid repeating chat content.
- The trash can permanently deletes only the selected area when a selection exists; if nothing is selected, it confirms clearing the whole board.
- General chat no longer runs full SOP / live hardware detection, reducing the fixed wait before the first reply.
- News-summary Chalkboards now extract up to 5 short headlines and 1 trend line, instead of clipping the body of the first news item.
- The front door and `app.js` use no-store, so updated auto-Chalkboard logic will not be overwritten by old cached versions.
- Short follow-up phrases for news queries now continue the previous filter; for example, after “latest game news,” saying “PC only” automatically switches to “latest PC game news.”
- Chalkboard supports multiple pages; the trash can only deletes the current page, while the other pages remain. Steam / PC new release tables are turned into 5 games plus 1 summary line.
