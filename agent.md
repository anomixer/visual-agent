...
## 📌 v1.0 — UI 重構：多分頁視窗與互動強化 (2026-03-17)

### 瀏覽器風格多分頁介面 (Multi-tab Interface)
- **Chrome Style Tabs**：中間主區域重構為多分頁結構，支援分頁切換、開啟與關閉。
- **🎨 Chalkboard (預設分頁)**：作為系統主儀表板，固定位於第一個分頁，不可關閉。
- **📋 工作清單 (動態分頁)**：僅在有任務或主動開啟時啟動，支援分頁標籤上的即時任務數量徽章 (Badge)。
- **自動切換邏輯**：當透過 AI 或推薦清單加入任務時，UI 會自動開啟並「跳轉」至工作清單分頁。

### 菜單欄位整合 (Menu Bar Integration)
- **實作「檢視 (View)」菜單**：頂部菜單欄現在可展開，列出所有可開啟的分頁（Chalkboard / 工作清單）。
- **分頁補位機制**：若工作清單分頁被關閉，可隨時從檢視菜單中點擊重新召喚。

### AI 對話與多媒體體驗優化
- **Markdown 渲染處理**：導入 `marked.js`，讓 AI 的回覆支援標題、列表、程式碼區塊等多層次格式，閱讀體驗大幅提升。
- **語音朗讀輔助 (TTS)**：
  - AI 訊息旁新增「小喇叭」圖示，點擊即可宣讀內容。
  - **過濾機制**：朗讀前自動剔除 `[ACTION:...]` 標籤與 Unicode 表情符號（Emoji），確保語音自然。
- **對話簡潔化**：修改 System Prompt 讓 AI 回答更精簡直覺，減少冗長解釋，直接切入核心。

### 模型與權限管理
- **模型效能升級**：預設模型從 `qwen3.5:0.8b` 提升至性能更強大的 **`qwen3.5:4b`**，並同步更新文檔與下載腳本。
- **搜尋增益**：右上角模型切換菜單加入清單搜尋欄，方便使用者在多模型環境下快速選取。
- **主題切換圖示優化**：按鈕圖示現在會依據當前模式智能變換（暗色顯日、亮色顯月）。
